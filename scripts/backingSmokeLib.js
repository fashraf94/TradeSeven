...
import { seatedIdsFor } from '../api/_utils/backingPools.js';
...
/** The marker every document the seeder creates carries; the cleanup verdict demands it. */
export const SMOKE_TOOL = 'scripts/backing-smoke.js';
 ...
 */
export function buildSmokeGroup({ ids, nowIso, userPool }) {
  ...
  return {
    ...
    smoke: { tool: SMOKE_TOOL, stamp: ids.stamp, createdAt: nowIso },
  };
}
 ...
 * strictly highest composite, and everyone else distinct values below it —
 * no ties, so the winning set is exactly the winner.
 */
export function buildSyntheticWeek({ group, battleMondayEtDate, winnerOdUserId, recordedAtIso }) {
  ...
  for (let day = 1; day <= 5; day += 1) {
    ...
    members.forEach((id) => {
      ...
      // non-zero too, so the week reads as played on both layers.
      const agentPoints = round2((human ? 4.0 : 1.5) * day - rank * 0.4 * day);
      closeScores[id] = {
        totalPoints,
        agentPoints,
 ...
 *   backingStakes/<id>[/private/meta]    the (parent) document's groupId is this run's pod
...
export function parseArgs(argv) {
  ...
  for (const a of rest) {
    ...
    else if (a.startsWith('--pod=')) flags.pod = a.slice('--pod='.length).trim() || null;
