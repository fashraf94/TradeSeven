...
// The DEEP-DATA set — the 20 names that carry a knowledgePackage / ledgerExtract
// / deepResearch bundle (Tier-1 context). STOCK_DATA is built for these only.
const DEEP_DATA_TICKERS = [
  'NVDA', 'AAPL', 'MSFT', 'AMZN', 'META', 'GOOGL', 'TSLA', 'AMD', 'AVGO', 'SNOW',
  'JPM', 'C', 'GS', 'MS', 'V', 'AXP', 'BX', 'AFRM', 'PNC', 'ALLY',
];

// The earnings/recap UNIVERSE emitted as `TICKERS` (Doug universe-expansion,
...
const RECAP_UNIVERSE = [
  ...
  'PG', 'NFLX', 'BAC', 'CRM', 'KO', 'CVX', 'AMD', 'ABBV', 'MRK', 'PEP',
  'ADBE', 'LIN', 'GE', 'MCD', 'NKE', 'WFC', 'TMUS', 'DIS', 'CAT', 'GS',
  'MS', 'AXP', 'RTX', 'HON', 'LOW', 'BKNG', 'ISRG', 'SPGI', 'NEE', 'PLD',
  'AMT', 'C', 'COP', 'SLB', 'BA', 'UNP', 'DUK', 'SO', 'PLTR', 'CRWD',
  'COIN', 'SNOW', 'AFRM', 'FCX', 'NEM', 'EQIX',
];

const BATCH1_TICKERS = DEEP_DATA_TICKERS.slice(0, 10);
const BATCH2_TICKERS = DEEP_DATA_TICKERS.slice(10);
...
const output = `// STOCK_DATA + helpers auto-generated by buildStockData.js — ${new Date().toISOString().split('T')[0]}.
// TICKERS is the earnings/recap UNIVERSE (${RECAP_UNIVERSE.length} names, curated subset of
// rankingConfig.js ALL_TICKERS spanning all 11 GICS sectors); STOCK_DATA is the
// ${DEEP_DATA_TICKERS.length} deep-context "Tier-1" names. The two differ ON PURPOSE — a TICKERS name
// without deep data degrades to the Tier-2 Sonar brief (api/stocks/analysis.js
// gates Tier-1 on STOCK_DATA membership); getStockContext still throws on an
// unknown key by design. See the arc note in the committed file's header.
//
...
export const TICKERS = ${JSON.stringify(RECAP_UNIVERSE)};
...
export function getStockContext(ticker, realtimeData = '', options = {}) {
  ...
  if (!stock) {
    throw new Error(\`Unknown ticker: \${ticker}. Available: \${Object.keys(STOCK_DATA).join(', ')}\`);
  }
...
// === SUMMARY ===
console.log('\n========================================');
console.log(`  BUILD COMPLETE — ${DEEP_DATA_TICKERS.length} DEEP-DATA STOCKS`);
console.log('========================================');
console.log(`STOCK_DATA (deep) bundled: ${DEEP_DATA_TICKERS.length}  ·  TICKERS (recap universe) emitted: ${RECAP_UNIVERSE.length}`);
...
console.log('  --------|-----------------|-----------------|-------------');
for (const t of DEEP_DATA_TICKERS) {
  const s = stockData[t];
...
const avgQuick = DEEP_DATA_TICKERS.reduce((sum, t) => {
  const s = stockData[t];
  return sum + s.knowledgePackage.length + s.ledgerExtract.length + 200;
}, 0) / DEEP_DATA_TICKERS.length;
const avgDeep = DEEP_DATA_TICKERS.reduce((sum, t) => {
  const s = stockData[t];
