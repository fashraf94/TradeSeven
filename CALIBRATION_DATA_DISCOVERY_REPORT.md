...
**Live numbers supplied by Flash (parallel pull):** **22 `agentBattles`, createdAt spanning ~March 27 – May 22, 2026 (~8 weeks).** Counts for `battles` (BaggerBomb V2/V3/V4) and `seasonEntries` were not supplied; `seasonEntries` is presumed ~empty (see §2.4). All recommendations below are framed conditionally against these numbers.
...
- The frozen `initialPortfolio` shows star/core/support (`:139-141`); **whether it also freezes `bench` is unconfirmed in code** — verify on a live record.
...
Bench schema is solid and sector-tagged where present, but it is **fragmented across two collections with different nesting, has a hard V4 discontinuity (no bench), and may have a pre-Firestore localStorage tail.** The 22 `agentBattles` are the clean, sector-tagged core. **Caveat for sector calibration:** those 22 were generated by the **existing 6-archetype enum**, *not* the new "defensive archetype" — so they are an anchor/realism set, not a direct "defensive archetype produced this book" sample (see §5, §6.1).
...
### 2.4 ⚠️ Season Mode appears built-but-not-shipped → presume ~empty
`useActiveDeployments.js:54`: *"TODO: Add seasonEntries query when Season Mode ships."* Reads are not wired into active deployments; a `SeasonModeToggle` and solo-season scaffolding exist, but consumption is gated. **Strong signal that `seasonEntries` is empty or near-empty in production.** Must be confirmed with a live `count()`. If empty, Season Mode cannot serve any calibration regardless of schema fit.
...
3. **Bench coverage of the 22 battles:** spot-check 2–3 `agentBattles` records — confirm `portfolio.bench` is populated and sector-tagged, and whether any are V4/bench-less or opponent-null (§1.3).
4. **`seasonEntries` count:** confirm whether Season Mode has shipped/produced any data (§2.4). If non-empty, it becomes a strong *sector*-calibration source.
