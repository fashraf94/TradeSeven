...
51. **The motive's author leads on the RATIONALE and falls back to `source`** (review L1-F3 / L1-F4, then FIX-1). `isEngineAuthoredMotive(text, source)` matches three prefixes — `Guardrail override`, `Risk manager:`, `Deterministic guardrail enforcement`, each pinned to its writer by a source tripwire — and then treats any `source` outside `{haiku, guardrail}` as engine-authored. The two exceptions are the sources where the text genuinely is the only answer: `haiku` (the model's, unless the cron overwrote it) and `guardrail` (the cron's sentence on a forced exit, the model's on a `reinforced_haiku`). An `evaluations[]` entry carries no `source` at all, so the panel and the check card use the text alone — which is correct, it is all they have. **A source added to the cron after today is engine-authored here by default**, which is the safe direction under C1.
...
- **Full suite:** 574 files, **9819 passed**, 63 skipped, 0 failed (9642 before the review pass).
...
Five isolated lenses and two refuters — the second pointed at the FIXES rather than the build, which is where the worst remaining defect was found. **50 findings raised, 35 CONFIRMED, 28 fixed on this branch**; 65 source mutations executed. The full record is the file above. What the founder should know:

1. **A ruling was implemented wrong, the review put it back, and a refuter then caught the correction over-rotating.** A2.2 discriminated the motive's author on `trades[].source`; ruling 5 named the **rationale**, and two independent findings proved the ruling right — a guardrail-forced swap that **executed** rendered the cron's sentence as the agent's words on one card while the card beside it called the same string the system's. Routine, not exotic: any user stop-loss that fires and executes. The first fix used the text alone, and a second adversary showed that inverted the safe default: the gameplan-rotation writer composes a template with no fixed prefix, so a system sentence became `The agent's own words` — which the original `source` rule had got right. **The rule that ships uses both**: the text leads, `source` rules out the writers whose sentence matches no prefix, and only `haiku` and `guardrail` let the text decide alone (those two being where the text genuinely is the only answer). Listing the two exceptions rather than the engine sources keeps the default safe for any writer added later.
...
3. **A2.1b was not revertible in isolation** — the reason D-76 gave for making it its own commit. Reverting it auto-merged away a declaration A2.2 consumed and left every trade card throwing, with no conflict to warn anyone. It is now **loud rather than silent**: the revert conflicts in five files and the resolution is to keep `tierLabel` and drop the A2.1b copy helpers, after which the suite is green. Measured by performing the revert, not asserted — an earlier version of this handover claimed it applied cleanly and that was wrong.
