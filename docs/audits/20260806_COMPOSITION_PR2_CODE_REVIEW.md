...
- **decide.js DERIVED classification ACCEPTED** — census `founderReviewFlag` resolved with the ruling recorded (disclosure 8 closed); the PR-4 splice remains available but is not required. *(SUPERSEDED same day by the Sol design-review adjudication: DERIVED stands for dark operation only; the PR-4 fenced splice is REQUIRED. Ruling of record: `docs/composition/ACTIVATION_PRECONDITIONS.md`.)*
