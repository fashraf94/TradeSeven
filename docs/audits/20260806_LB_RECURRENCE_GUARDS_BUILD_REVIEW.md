...
**6a · FOUNDER DECISION (B-F3) — the §7.2 day-5-carried week, post-clamp.** The old self-heal (bank day6 fresh → gate clears) is retired by Guard 1 — correctly, since that heal WAS the contamination. But its retirement leaves: (i) four stale self-heal comments/logs (`leagueTournament.js` §7.2 docstring; `tournamentAdvancement.js:333-334,:639-640,:449-450`); (ii) circular operator guidance in the one scenario the guards exist for (banking says investigate advancement; advancement says banking will heal); (iii) no sanctioned recovery for a day-5-carried week (the manual bank endpoint is clamped too). Ruling needed: **bless "day-5-carried ⇒ paused pending manual intervention"** (then fix the four texts + differentiate the two logs — advancement text edits sit in the frozen path's neighborhood, hence not done unilaterally), **or sanction a heal path** (e.g. a re-bank-day-5-fresh mechanism — new surface, out of L-B scope). Either way before L-C unfreezes; nothing is wrong tonight (no BATTLE group is day-5-carried).
