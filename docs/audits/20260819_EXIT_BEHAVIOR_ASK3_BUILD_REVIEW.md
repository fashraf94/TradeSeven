...
## 8. Post-push CI fix + process note (module-scope flag reads)

**The failure.** CI red on the branch: **7 test files failed to LOAD** (not a logic defect) — `api/agent/`: `change-archetype.compat`, `change-archetype.leanrider`, `equip-bundle.compat`, `equip-lean.behavior`, `masteryEnforcement.behavior`, `reforge-bundle.compat`, `set-tempo-dial.behavior`. Root cause: `compileBuild.js` reads `PROFIT_TARGET_EXECUTOR_ENABLED` at **module scope** (the frozen `SUPPORTED_GUARDRAIL_SHAPES` / `STRICTEST` exports, `:99`/`:112`). Those 7 suites use **hermetic whitelist** `vi.mock('featureFlags', () => ({...}))` factories that enumerate only the flags they pin (each carries the identical `COMPILER_ENABLED: false // keep the dark compiler dark` intent, several with getter-bound per-test `flagState`). Under Vitest, reading a named export absent from such a factory **throws at access time**; because the access is at module load (via `compileOnSettingsChange.js:41` → `compileBuild.js`), the whole file fails to import. Tally: 7969 passed / 0 failed / **7 files failed to load** — a pure load-time break.

**The fix (deliberate, per-file — not blanket).** All 7 are the *same case*: deliberate hermetic whitelist mocks, one homogeneous test family. Each got an explicit `PROFIT_TARGET_EXECUTOR_ENABLED: false` added to its factory — **not** an `importOriginal` conversion. Rationale: (a) explicit-`false` preserves the intentionally-minimal flag surface (an `importOriginal` spread would leak every *real* flag value into a mock deliberately built to exclude them — the exact "spread picks up a different value / assertions shift" hazard to avoid); (b) `false` is the dark/production value, so the compiler takes the same dark path (`profitTarget` absent from the shape table) these suites already exercised before Ask 3 existed — **zero assertion shift, confirmed**: all 74 tests across the 7 pass identically. `importOriginal` is the right tool for *incidental* overrides; these are not incidental.

**Sweep (same class).** Repo-wide, ~15 test files mock `featureFlags` with an explicit-object factory (no `importOriginal` spread); the 7 above are the subset that transitively reach the module-scope read today. The rest — `ensure-casual-clone`, `log-rule-compat-event`, `set-rule-hardness.compat`, `live-draft-fire`, `lobby-endpoints`, `slot-endpoints`, `TradeHistorySection.motive.render`, `ruleCompatMatrix` — do **not** currently reach one, so they stayed green; they remain the **same latent class** and the *next* module-scope flag could break any that reach it. **Not fixed here** (not breaking, outside Ask 3 scope, and converting tests I don't own is unrelated churn) — filed as a separate-tasking item: migrate the explicit-object `featureFlags` mocks to the `importOriginal` spread. A **second** module-scope read was introduced by this build — `StrategyDimensions.jsx:144` (inside the module-scope `DIMENSION_CONFIGS`) — but **no test imports that component**, so it is latent, not live; kept (it's a legitimate build-time read and keeps the Ask 1 flip copy-free), reported here.

**Why STOP-2 verification missed it — the real lesson.** The full suite *was* run at STOP-2, and it *did* report the failure (`Test Files: 7 failed`, exit code **1**). The miss was operational: the run was piped through `tail`, which (1) replaced Vitest's exit code with `tail`'s `0`, and (2) truncated *past* the `Test Files:` summary line, leaving only `Tests: 7973 passed | 0 failed` — a line that counts individual test results and therefore **does not reflect file-load failures**. A red suite was read as green. **Two standing rules from this:** (i) whenever a flag enters module scope, every hermetic `featureFlags` mock must list it (or spread `importOriginal`) — a module-scope read reaches distant mocks a touched-surface battery never imports, so a *full-suite* run is mandatory; (ii) never pipe the suite through `tail` — assert the process **exit code** and read the **`Test Files`** line, both of which correctly surface load failures that the per-test tally hides. Post-fix full suite: **491 files passed / 0 failed, exit 0; 8,047 tests passed** (the +74 vs pre-fix are exactly the 7 recovered files). `vite build` ✓; the 7 changed files lint clean (the repo's `eslint .` baseline of ~1,563 pre-existing `no-undef`/unused-var errors is unrelated — this diff adds none).
