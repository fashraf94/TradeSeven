...
**Accepted cost of the deferral.** The mislabel is latent **only while writes are off**. Once `WIRE_WRITES_ENABLED` flips, Kim's `sector_rotation` entries persist a `sector_vs_spy` magnitude asserting an *uncomputed* relationship — the seam populates agentFacts under writes (`generate-column.js:288,306,403`), the validator accepts the basis (`wireValidator.js:252`; `wireContracts.js:397`), and the digest renders "…% vs SPY" on a raw sector move (`wireDigest.js:37`). It is **gate-harmless** (adapter → NOT_VERIFIABLE, excluded from the scored denominator — `wireEditorialAdapters.js:379-380`, calibration §4) but writes **false records into the Wire**, agent-visible once the Wire is consumed (post-Phase-3). The founder accepts this as the price of the post-gate deferral.

**Stopgap if the flip-to-arc gap runs long (advisory).** If `WIRE_WRITES` flips before the (A) arc lands, a considered interim is to **suppress Kim's magnitude entirely** — declare `eventType` + `direction`, omit the basis — so nothing false is asserted while the real fix waits (magnitude is optional, `wireValidator.js:246`; digest drops the clause, `wireDigest.js:78`). Keeps Kim gate-silent but honest; itself a small generation-surface change (epoch bump), so also post-gate — a smaller honesty-only touch than (A), not a substitute.
