...
**Adversarial verification (independent reviewer, 6 axes — SOUND, no defect):** correctness (voided dropped incl. mid-day; live/casual/unresolved kept); fence (read-only — `voidGroup` writes only the group doc, and the client `tournamentGroups` read is explicitly allowed at `firestore.rules:449-451` and auth-gated, so **the propagation is real in production, not a silent no-op**); fail-safe (per-group `try/catch` → fail-open; a rejection cannot escape to the outer catch, which never blanks state); regression (no `limit()`; runs after the clone filter; no-tournament case = zero extra reads, identical to before); consistency (the battle-weather sky reads the same filtered state; the member voided-card is fed by a SEPARATE `subscribeMyMostRecentVoidedGroup`, so the exclusion is complementary, not self-hiding); test integrity (real mutation check + meaningful source guards). Three non-defect observations (stale prose comments in two unrelated test files; the write-guard regex omits `writeBatch`/`deleteDoc`; the new `getDoc`s aren't `trackRead`-counted) — all no-action.
