...
| **Does merging change anything live?** | **No.** The switch (`CALL_RECORDS_MODE`) ships **off**. With the switch off, the following are proven byte-identical to a snapshot taken before the change, on 19 battle scenarios: every battle write, every prompt byte, the AI's tool definition, both capture documents, and every argument passed to the trade executor, ledgers and narrators. |
...
|---|---|---|
| At off, byte-identical to the frozen pre-change fixture under injected clocks and ids: the tool schema, every battle write on every exit, every prompt byte, both capture outputs, and every argument handed to the executor, ledgers and narrators | `agent-evaluate.callRecords.offGolden.test.js`: 19 scenarios × updates/prompts/capture/writer arguments/summary/thrown; golden SHA-256 pinned | M1a, M1b, M1c, MSa, RC2 |
