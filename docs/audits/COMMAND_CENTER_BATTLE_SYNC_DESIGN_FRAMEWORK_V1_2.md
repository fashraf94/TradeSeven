...
**Date:** August 31, 2026
**Status:** Design framework, pre-spec. **Supersedes V1.1** (and, through it, V1, Amendment A, Amendment B). Locked items are marked LOCKED. Open items are in §15. All proposed rulings are confirmed (§0). Amended in place Aug 31 (evening) for D-20, D-21, §7.1; **amended Sep 2 with D-42 → D-62 and §15.1 corrections of record**; **amended Sep 2 (evening) with D-63 → D-79** (Phase A2's rulings); **amended Sep 3 with D-80 → D-85** (the A2.3/A2.4 rulings addendum V1.1); **amended Sep 3 (evening) with D-86 → D-90** (the flip-prep seed); **amended Sep 4 with D-91 → D-98** (the A3 character-pane seed, as renumbered by its Phase 0 report §9 and ruled by the Sep 4 rulings) (arc authority now the design chat); **amended Sep 4 (evening)** — D-92's shape and D-97's status, after the founder's first smoke and the A3.6 build; **amended Sep 8 with D-99 → D-105** (the voice-layer grounding Phase 0 build G1 → G6 — `docs/design/VOICE_LAYER_GROUNDING_SPEC_V1_2.md` §11 and the Sep 7 rulings; D-55, D-76 and D-86 amended in place; the build review is `docs/audits/20260908_VOICE_GROUNDING_BUILD_REVIEW.md`). **Amended again Sep 8 with D-106 → D-109** (the walk's rulings — `docs/design/VOICE_LAYER_GROUNDING_SPEC_V1_3.md` Appendix D; V1.3 supersedes V1.2). **Amended Sep 9 with D-110 → D-115** (Phase B — the tick stamps; `docs/design/PHASE_B_TICK_STAMPS_SPEC_V1.md` §6 as amended by its V1.1 errata, the B1 server build's review findings, and Sol's Sep 9 rendering pass — `docs/audits/20260909_SOL_REVIEW_PHASE_B_TICK_STAMPS_PASS.md`).
...
| D-14 | 3-day flip — fenced constant, inactive path, budget reset; **and the ET calendar-day grouping the whole check-naming family needs** | Not this arc | Own gated ruling after beta proves the surface on fullday. **Prerequisite recorded Sep 4, 2026** (founder ruling on the A3 review's open question 1): `evaluations[]` is never reset by day, and every surface that names a check names it by SLOT alone (`slotLabel`, D-83) — so on day 2 of a 3-day battle Bench, the check cards, the book panel and the quiet-check runs would all label yesterday's entries as today's, and `No check yet today` would be unreachable after the first decided check ever. Under fullday this is unreachable and nothing is built for it. When 3-day is specced, the WHOLE family groups entries by ET calendar day and labels prior days — one change across all of them, because a day-aware Bench alone would disagree with the card above it. Bench keeps its scan-back (D-92) either way. |
...
| **D-76** | **The plan at deploy may render** — the brief on the book panel; a tier rationale's sentences that name the symbol on a row (`At deploy · {tier} tier`), else nothing. Gated off tournament battles (`gameMode`) and the algorithmic fallback template (`innerMonologue.strategy` begins `Algorithmic selection`); every label carries `activatedAt`. Built as its own commit so it can be reverted in isolation. | C1, Phase 0 §2.4 | **Founder's lean, ruled Sep 2 (rulings §1 ruling 4). SUSTAINED WITH A RULE by Sol's pass (spec V1.2 §11, Sep 7):** the exact persisted deploy artifact, code-rendered, labelled `The plan at deploy · {date}`, gated off tournament battles and the template fallback, never as current intent. Built A2.1b (`selectDeployPlan.js`, commit `ff98084f`); the narrator's lazy opener renders the same artifact under the grounding flag (`buildPlanAtDeployBlock`, `voiceLayerGrounding.js`; the gates shared through `src/data/decisionRecord.js`). Closed. |
...
| **D-86** | **Every tape entry says what KIND of thing it is, from the persisted type — never from the text.** `Status check · {t} · {state}` on a check card ({t} the cron slot, D-83); `Bench note` (`anticipation`, `potential_entry`), **`Holding note` (`anticipation`, `potential_exit`) — the second word, ruled by the Sep 7 adoption of spec §5 and rendered ONLY on an exchange produced under the grounding contract (`groundingVersion: 1`); a legacy model-written exit note keeps no eyebrow, exactly as shipped, so the flag-off page is byte-identical (build review R-02 — one line in `battleViewCopy.js` reverses it)**, `Trade note` (`trade_narration`), `Opener` (`first_message`), `Reply` (a `user_initiated` exchange that HAS a user half), `Directive` as shipped. `auto_debrief` keeps its shipped `Post-Market Debrief` and gains no second eyebrow. **An unknown type renders NO eyebrow** — a new server type reaches the design chat before it reaches the screen. **The chip filing's audit exchange (`messageType: 'directive_filed'`, `file-directive.js`) is such a new type, ruled here: it carries no narrator words, renders no eyebrow and no bubble body — the ExecutionCard's own `Directive` eyebrow (D-68) is its word.** The two absence labels (D-65, D-69) keep `{t} · {label}`: they already end in "at this check", and rewording a ruled string to fit an eyebrow would be a ruling. | D-84, D-83, hazard 24 | **Ruled Sep 3** (flip-prep seed item 2). Built flip-prep session (`battleViewCopy.js`, `AgentChat.jsx`, `deriveChatMessages.js`). |
...
| **D-92** | **Bench quotes the decider only.** Bench lists the bench roster — the union of `portfolio.bench.{stocks,crypto}`, `watchlist.hotBench[]` and `agentContext.equippedWatchlist.tickers[]`, **minus the book**, deduped, in list order — under the equipped watchlist's bare name, and renders the last DECIDED check's `rationale` **sentence-first**: each sentence that names at least one bench symbol appears ONCE, verbatim, in the rationale's own order, carrying the names it mentions as chips; the roster it did not name is one `Not named at the {t} check` line and one wrapped row of chips beneath it. One split, then ONE PASS OVER THE SENTENCES — through the same `splitSentences` / `namesSymbol` / `parseEmphasis` path the row, the tape scope and the check card already share (D-87), so Bench is the fourth consumer, never a copy. **Amended Sep 4 (evening), after the founder's first smoke:** the first build grouped by SYMBOL and so reprinted one sentence naming three bench names three times, once under each, and repeated `Not named at the {t} check` beside every roster name — the decider's paragraph shredded into a per-name index of itself, which is what made the section read as a list rather than a bench. **On an outage tick Bench SCANS BACK** to the last check whose entry carries a rationale and labels that slot — in the HEADING, as `Named at the {t} check`, naming the check actually used rather than "the last check", which the scan-back can make false (founder ruling Sep 4). The day bound that string implies is a D-14 prerequisite, recorded there and unreachable under fullday; `selectLatestDecision` is null on such a tick and would otherwise claim a silent day (hazard 40). The absence line renders only when no entry today carries words. A narrator exchange can never reach Bench: the selector's only text input is `evaluations[].rationale`. No bench price is polled — the mock's per-name `%` has no source and is not built. | Phase 0 §2.5, §2.6, §4 #11, hazard 40 | **Ruled Sep 4** (A3 rulings §2 #11). Built A3.3. |
| **D-93** | **The strip and the sheet retire under the pane flag (D-74 superseded).** The pane replaces the desktop `PeekStrip` column and the mobile `ChatSheet` at the screen's six wiring sites; each site branches `paneOn ? … : <the A2 JSX as merged>`, so pane-off renders A2 byte for byte — a claim the five-lens review tested in states the first-paint golden cannot photograph, and which needed one correction to hold (the hoisted scope chip had carried `flex-shrink: 0` into the A2 position too). `useChatSheet` stays **called** with `enabled = controllerOn && !paneOn` — disabled it reads peek and resets to peek, so hooks order never changes and nothing A2 can render open beneath the pane (hazard 44). The pane gets its own pure machine, `useCharacterPane`. The one `AgentChat` keeps **one tree position** across collapse, expand and section changes: Bench and Tape HIDE the Chat section, never unmount it, or the draft, the in-flight send and the scroll are lost (hazard 45) — **and so does COLLAPSE**: the review found the first build honouring the sections and unmounting the pane on a fold, which lost the draft on exactly the transition hazard 45 names first. The pane is hidden, never unmounted, on both. | Phase 0 §2.1, hazards 44–45 | **Ruled Sep 4** (A3 rulings, §4). Built A3.2. |
| **D-94** | **Game Tape and the watchlist chip leave the header; Tape is a pane section with its bookmark control.** Under the pane flag the header link, the overlay and the chip are not rendered — `gameTapeOpen` stays false and its five consumers are inert without an edit — and the watchlist's bare name moves to Bench's subtitle from the same field. Tape renders trade cards through the shipped `TradeCard` over the same `tapeEntries` the chat renders, and keeps the **shipped** add / remove bookmark control (a moved client write, shipped behaviour) and the activity log as they are — BOTH halves: the review found the first build moving only `remove`, which left the feed's own `Add bookmark` control rendered and wired to nothing. The header's bookmark dot becomes a **count on Tape's own section header** (`Bookmarks · n`) and appears nowhere on the board. | Phase 0 §2.3, §4 #12 | **Ruled Sep 4** (A3 rulings §2 #12). Built A3.4. |
...
| **D-97** | **Motion marks EVENTS, never states.** The bagger burst keys on the **persisted** transition — `thresholdHistory[sym].maxMultiplier` `< 1.0` on one snapshot and `≥ 1.0` on the next, seeded silently on the doc's first snapshot, a decrease re-seeding and never firing — and never on the live merge, or a price flicker fires it twice (hazard 37). The row's `BAGGER` tag **is the shipped badge**, live-merged as it ships; the burst, the footer and the bubble are the persisted-only additions. `ChamberFuse`'s 400 ms flash stays and marks the **price** crossing: two events up to a tick apart, not one event shown twice (hazard 38, recorded). `Bagger hit · {mult}× banked` reads the row's **conviction tier multiplier** (`2× / 1.5× / 1×`) — the number the player is playing for; `banked` stands because the history is monotonic and the tier bonus banks by construction. `Bagger · {sym} hit {pct}` reads the **bagger line** `+{baseATR}%`, the persisted threshold the row's `%` reads at that price. **Bagger only in A3.6**; double and ten are the same path with a different constant, ruled later. The avatar never moves between events; no new CSS keyframe. | Phase 0 §2.4, §4 #7–#10, hazards 37–38, 47 | **Ruled Sep 4** (A3 rulings §2 #7–#10). **Built A3.6** (Sep 4, evening). The footer is derived per row from the persisted peak rather than held with the moment — a line that says `banked` must not depend on whether the tab was open when the tick landed — and the bubble is `standalone`, because `Bagger · {sym} hit {pct}` is not a tape entry and carries no unread count to be gated on. |
| **D-98** | **The bubble is sharp and unstriped, and its eyebrow shares the stream's kind colours.** No tail, no fill stripe, no `14px 14px 4px 14px` corner — the literal `4` and nothing else; the eyebrow carries the kind as **text**. It is composed of nothing: the eyebrow comes from `tapeKindEyebrow` and the line from the `derivePeekLine` helpers, never assembled by hand. Its colours are the record edges' existing `LABEL_COLOR` for records and `text-muted` for speech — and **the chat's own eyebrows share those colours** — which the build achieved WITHOUT the gated `AgentChat.jsx` change this row first described, because that change would have been a no-op: the chat's kind eyebrow is only ever rendered on agent SPEECH (`AgentChat.jsx:267`) and is already `text-muted` (`:302`), which is what the speech rule assigns it, while the stream's records are `TapeCards`, which owns `LABEL_COLOR`. `TapeCards` now EXPORTS its four eyebrow colours and `deriveBubble` imports them, so the bubble and the card cannot disagree about a kind by construction rather than by two matching values (hazard 43; the substitution was put to the review's fifth lens, which verified the premise and confirmed the mechanism). The fade is asserted by attribute or state, never by timing (hazard 47). | Phase 0 §3 item 11, §4, hazards 43, 47 | **Ruled Sep 4** (A3 rulings §3). Built A3.1. |
| **D-99** | **The hypothesis field renders, with a rule.** In YOUR RECORD (spec §3.2) the field renders as `Hypothesis recorded at this check (graded after the battle): {hypothesis}` — only when present, only when the rationale does not already carry it under the §3.2a duplicate normalizer (the known bold-vs-field pair), never where the rationale is engine-authored (the cron writes its own hypothesis beside its own rationale on the guardrail path), and never voiced as the narrator's forecast (the RATIONALE RULE printed beside the block). The pane's A3.7 rendering (`Hypothesis recorded at the {slot} check · graded after the battle`, spec §11) shares the rule; when A3.7 lands the label moves to `src/data/decisionRecord.js`. Hazard 29's "never render hypothesis" is amended. | spec §3.2, §3.2a, §11; hazard 29 | **Ruled Sep 7** (spec §11, Sol's pass adopted). Built G2 (`voiceLayerGrounding.js`: `HYPOTHESIS_LABEL`, `rationaleCarriesHypothesis`, `renderRecordEntry`); the engine-authored exclusion from the build review (R-12). |
| **D-100** | **Assignments V2's conversational trigger — sustained with a rule (D-55 amended).** A scouting ask from the cited record is R1-compliant; symbols from the actual record only; research/equip only; no prompt-assembly entry except the ruled equip path. | D-55, spec §11 | **Ruled Sep 7** (spec §11). Nothing built in the grounding build; D-55's status cell carries the amendment. |
| **D-101** | **`VOICE_GROUNDING_MODE` is a string tri-state, pinned directly, walked by the founder.** `'off' \| 'shadow' \| 'canary' \| 'on'`, shipped `'off'`; pinned DIRECTLY in `src/config/voiceGroundingFlags.test.js` with the `// Pinned by:` pointer, OUT of `DARK_BY_DESIGN` (the guard scans `*_ENABLED` booleans only), with NO boolean companion (ruling 3). The canary allowlist is the env var `VOICE_GROUNDING_CANARY_UIDS`, parsed at CALL time inside `getVoiceGroundingMode(uid)`; unset or empty = nobody; never a repo constant (ruling 2). `'canary'` resolves to `'on'` for an allowlisted uid and `'shadow'` for everyone else; an out-of-vocabulary value resolves to `'off'`. Every writer reads the mode per caller at call time — the battle OWNER's uid for the crons and the lazy opener, the TOKEN's uid for the chat turn and the filing route. The walk `'off'` → `'shadow'` → `'canary'` → `'on'` is one one-line PR per step reconciling the pin (BUILD_RULES §2); rollback is the same literal walked back. | spec §9; rulings 2, 3 | **Ruled Sep 7.** Built G1 (`featureFlags.js`); the off goldens captured from the post-ATR-pick tree and proved by reconstruction (`voiceLayerPrompt.grounding.goldens.test.js`, sixteen surfaces). |
| **D-102** | **The grounded prompt is inputs, not authority; the history window has one rule.** Under the flag the battle prompt carries the §3.1 identity frame, YOUR RECORD (the last three checks newest first, verbatim bytes, every rationale labelled with its author — `The system's reason` for an engine-authored motive — outage entries as the D-65/D-69 absence line only, the pane's own state selector over the SAME shared gates so a guardrail-forced swap that failed to execute reads `A guardrail called for a swap · it did not go through` on both surfaces), CURRENT CONTEXT (the cache blocks under one heading with the vintage sentence, in the opener too; the weekly fundamentals line per name, dated in ET, null-honest), the current directive through the ONE reader the narration shares, and the narrator's own earlier messages as a system-prompt block. The history window: a user-initiated pair enters the model's message array tagged `[messageType]` (the chat turn's type is the code default `user_initiated`, never persisted); an agent-initiated exchange rides the system block ONLY when it carries the top-level `groundingVersion: 1` marker; a chip filing (the user's tap, not the narrator's words) and every legacy proactive exchange — including the fenced deploy-time opener, which no writer can stamp — stay out (ruling 4). The 30-site vocabulary guard (35 phrases) is absent from every grounded surface and present in the off surface each names; the phase examples mint questions only. **Recorded, not built:** the deploy-time opener generated inside fenced `decide.js` stays the OLD prompt under `'on'` (only the lazy `ensure-opener` path is grounded) — a §7-gated one-key change or an accepted gap; and YOUR RECORD's verbatim bytes carry the cron's provenance code (`Guardrail override (guardrail_stopLoss): …`) that D-80 keeps off the screen — bytes (spec §3.2) or the pane's `renderMotive` (D-80) is a ruling. | spec §3, §4; ruling 4; hazards 23, 25, 26; D-80 | **Ruled Sep 7.** Built G2 (`voiceLayerGrounding.js`, `voiceLayerPrompt.js`, `openerTemplateFloor.js`; the shared strings extracted to the zero-import `src/data/decisionRecord.js` because `battleViewCopy.js` is not Node-clean — hazard 26); the fifth-state and opener-context fixes from the build review (R-01, R-13, R-15). |
| **D-103** | **The anticipation note is composed in code and names the event, never the plan.** Under the flag `generateAnticipation` makes NO model call: the exchange text is `At the {slot} check my trading process flagged {SYMBOL} on the bench as a potential entry` (or `in the book as a potential exit`), the slot from the evaluation entry's own timestamp; the signal clause is appended only when it passes the reply lint (`I'll rotate \| I'm rotating \| eyeing \| watching \| keep an eye \| I'd consider … swap`); `threshold` and `signalSummary` never reach the exchange (`anticipationContext { symbol, direction, evaluationId, slot }`); `suggestedActions: null`, `scratchpad: null`; one note per `(symbol, direction, ET day)` — deduped in-process on the dispatch queue and per candidate against the battle's own exchanges with `formatEtDate`. The trade narration is retired under `'on'` (no read, no model call, a breadcrumb); the trade card remains the notice. | spec §4, §5; Sol F1, F8, F10 | **Ruled Sep 7.** Built G3 (`voiceLayerAnticipation.js`, `voiceLayerTradeNarration.js`, `agent-evaluate.js`). |
| **D-104** | **A chip files through `POST /api/agent/file-directive`, one transaction, and the receipt is the exchange it wrote.** The route is live when the mode ≠ `'off'` (404 otherwise); eight checks in order (owner, active battle, agent-belongs-to-battle, the client's belief `expectedDirectiveThreadId` equals the raw slot's thread — 409 with the server's current thread otherwise — permitted for the SERVER-derived archetype with no fallback — 422 `off_menu` — canonical text server-side, the budget — 429); all reads precede the writes; the write is the shipped enforce-path shape from ONE module (`directiveFiling.js`): the slot with `adjustmentId` + `canonicalTextVersion`, and the audit exchange with `directiveThreadId` top-level AND inside `directive` (ruling 5), `messageType: 'directive_filed'`, `source: 'chip'`, `groundingVersion: 1`, `groupId` on a tournament battle; a prior directive — a legacy slot with no thread id included — makes the status `replaced-prior`. Spec §6.3's `Filed: {text}` line is withdrawn: the Battle View derives `Filed {time}` from the audit exchange through `deriveReceipts` → `receiptLine`; the arena renders the route's post-commit response with the same `filedLabel`; neither renders a bubble body for the filing. Chips are minted by id: the grounded OUTPUT_FORMAT asks for `{ kind: 'directive', id }` from YOUR MENU or `{ kind: 'ask', text }`; the server rewrites a directive chip's text to the canonical text and drops an off-menu, duplicate or unknown chip; a directive chip reads `Files: {canonical text}` and its tap goes to the route with the server-fed belief (the chat turn's `currentDirectiveThreadId`, then the filing's, a 409's adopted) — never to the chat; the chat turn's `directiveStatusLine` gains a client consumer on both surfaces (the Battle View from the persisted grounded exchange on which the gate ran and wrote no directive; the arena from the grounded answer). Gate internals stay private: the route and the normalizer call `getAllowlist` / `isValidAdjustmentId` / `getCanonicalText` and `getEffectiveArchetype` directly, and both new direct importers of the legacy table are recorded in the §2.3 baseline. Review-mode chips are out of scope. **Recorded:** the route has no review-mode gate — a directive filed after the close is in front of the process at the next check — and the HTTP status is the client contract. | spec §6, §8, §10; rulings 5, 6; Sol F2; D-18, D-60, D-68, D-90 | **Ruled Sep 7.** Built G4 (`file-directive.js`, `directiveFiling.js`) and G5 (`AgentChat.jsx`, `arenaEngineCore.js`, `useArenaEngine.js`, `CommandDock.jsx`, `VoiceLane.jsx`); the examples, the belief hold, the filing-in-flight guard and the legacy-slot status from the build review (R-03, R-16 → R-19). |
| **D-105** | **The filing's budget is server-derived from `battle.gameMode`, and a League filing charges the League store inside the same transaction.** `battle.gameMode === 'baggerbomb_tournament'` → the per-day `agentChatBudget/{groupId}_{uid}_{dayN}` document is read and written INSIDE the route's transaction (battle doc + budget doc; `resolveBudgetDay` called inside it; the same shape `chargeAgentChatBudget` writes); an unkeyable day fails open, the chat route's own contract (files for free, `remaining: null`). Any other battle charges `chatBudgetUsed` on the battle doc by an explicit in-transaction count (never `FieldValue.increment`), the cap authoritative under a race. The client never picks its budget: the body is `{ agentId, battleId, adjustmentId, expectedDirectiveThreadId }` and nothing else — no `leagueAsk` marker on a filing. Consequence recorded: on a League battle opened in the Battle View, a chat turn charges `chatBudgetUsed` while a chip filing charges the League day store, so the Battle View's own counter does not move on a League filing. | ruling 6; spec §6.1, §8 | **Ruled Sep 7.** Built G4 (`file-directive.js`); the two transaction writes recorded in `compositionProtectedStoresAllowlist.json` in G4. |
| **D-106** | **`file-directive` is reachable only at resolved `'on'`; `'shadow'` is measurement only.** (Amends D-102.) | spec V1.3 §6.1, §9; Appendix D | **Ruled Sep 8** (the walk; spec V1.3 Appendix D). Built: `api/agent/file-directive.js:172` gates on `=== 'on'` (404 otherwise). |
| **D-107** | **The `fundamentals` mirror at `'shadow'` is not fence contact:** the fenced reader's rendered prompt is byte-identical, proven by the off goldens; the walk stays under the flag's own protocol. | spec V1.3 §9 (`'shadow'`); Appendix D | **Ruled Sep 8** (the walk; spec V1.3 Appendix D). The proof is `api/_utils/voiceLayerPrompt.grounding.goldens.test.js` (the fixture carries the `fundamentals` field the off bytes ignore); the mirror is `api/cron/voice-layer-cache.js:819`. |
| **D-108** | **The shadow record is written durably from `chat.js`** (`waitUntil` or a clamped awaited write); the four sibling handlers on the same stream are separate tasking. | spec V1.3 §9 (`'shadow'`); Appendix D | **Ruled Sep 8** (the walk; spec V1.3 Appendix D). Built in `api/agent/chat.js` (`waitUntil` from `@vercel/functions`, `chat.js:13`, else the clamped await); the sibling handlers are not built here. |
| **D-109** | **System-authored rationale renders through the shared translator wherever it reaches the narrator or the player;** `Flat6BattleView` joins the shared renderer in its own PR. | spec V1.3 §3.2; Appendix D | **Ruled Sep 8** (the walk; spec V1.3 Appendix D). The translator is `renderMotive` in `src/data/decisionRecord.js:295`; `Flat6BattleView` has not joined it — its own PR. |
| **D-110** | **Heard: `{ directiveThreadId, suppressed }` stamped by the cron from the same `resolveControls` call as the assembler, on the in-memory object, GATED ON THE PROMPT HAVING BEEN BUILT AND HANDED TO THE TRANSPORT;** supports one claim — the thread was in the decider's prompt at that check; `suppressed` is not Heard. **Amended at build** (review A-4 / B-1): the gate is `promptBuilt`, not `haikuAttempted`, which is set BEFORE the prompt is built — a builder throw would otherwise have stamped a prompt that never existed. **The client (seed §1):** `Heard at the {slot} check` when `suppressed === null`; the reasonless, system-owned `Not heard at this check` when it is not; NOTHING when no entry names the thread (the mid-tick filing, and every pre-flip battle). **The four suppression reasons never reach a surface** (Sol M-1) — the character never received the withheld directive, so explaining the withholding in its voice attributes a pre-prompt resolver event to the character. In the narrator a withheld thread leaves the CURRENT DIRECTIVE line unchanged: no negative, no reason. | spec §1.2, §1.5; seed §1, §4; D-52; Sol M-1 | **Ruled Sep 8**; amended Sep 9 at build. Server: `api/_utils/tickStamps.js` (`deriveHeardStamp`). Client: the shared walk is `heardStamps` in `src/data/decisionRecord.js`, wrapped by `src/screens/battleView/deriveHeard.js` for the pane and read directly by `api/_utils/voiceLayerGrounding.js` for the narrator — ONE walk, so the card's line and the prompt's line cannot disagree (hazard 26, §9). |
| **D-111** | **The evidence stamp: EIGHT fields per held position plus one vintages block,** code-composed from the tick's own objects; bench, story ids and the weekly/daily duplicates cut; `risk`, `atrX`, `vwapDev` never cut; labelled as what the decider saw. **Amended at build:** NINE → EIGHT — `rsPct` is not stamped (review A-2: the prompt renders `rsPercentile` for BENCH names only; a held name's technical read reaches the decider as the `regime` word). `chg` is the ACTIVE POSITIONS row's **Gain% from ENTRY**, not the quote's session change (A-1). `tech: 'daily'` → **`techAt`, an INSTANT** (A-3: those docs are rewritten hourly during RTH, so no cadence word fits). `fundAsOf` is **the FUNDAMENTALS block's own header date** across held plus non-crypto bench (A-6), not a per-symbol timestamp. **The client (seed §2, §4):** `What the {slot} check saw` under the piece's Why? panel and `What this check saw:` in YOUR RECORD, both through ONE renderer (`evidenceFactLines` / `provenanceLine`, `src/data/decisionRecord.js`). **Three carve-outs, all Sol's:** risk `HOLD` is SILENT under the "saw" heading — the prompt renders no RISK STATUS block on an all-HOLD tick, so HOLD is a verdict conveyed by the block's ABSENCE (B-1, the blocker) — and the stored `reason` CODE is never rendered as seen text, because the prompt carried the `detail` sentence; `chg` is always labelled **since entry** (M-3); the provenance line NAMES its fields (`Latest held technical stamp`, `Fundamentals block as of`) and never promises freshness (M-2). **Eight means eight** (m-1): no ninth slot, no `RS unavailable` placeholder, no completeness rule; a null metric renders nothing, never 0. **The claim is VISIBILITY, never CAUSALITY** — no surface says a value caused a hold or a swap, and the grounding rules say so to the model. | spec §1.3, §1.5; seed §2, §4, §5; Sol B-1, M-2, M-3, m-1 | **Ruled Sep 8**; amended Sep 9 at build. Server: `api/_utils/tickStamps.js` (`composeEvidenceStamp`, `composeVintages`). Client: `src/screens/battleView/selectEvidence.js` + `WhyPanel.jsx`; narrator: `renderEvidenceLines` in `voiceLayerGrounding.js`, with a 300-token budget that drops the evidence to the NEWEST entry only when it would cost more. |
| **D-112** | **Candidates persisted on the entry (four fields plus tag); `threshold` persisted, never rendered in the narrator's voice (D-103); Bench renders the fact of the flag.** **Recorded at build (review B-5):** the stamp is UNCAPPED — the dispatch queue is equally uncapped and the fenced instruction expects 1–3 per day; **a first-five cap is a spec-level rule and is built in B2**, not here. **The client (seed §3):** `candidates` filtered to `direction === 'potential_entry'` intersected with the bench roster; a flagged name MOVES out of "the rest of the roster" into the `Named at the {t} check` group with a `Flagged` chip, in ROSTER order. **The fact of the flag is the whole payload:** `flagged` is a list of strings, so `signalSummary`, `threshold` and `signalSource` have no field to ride on and reach no surface. The word is a PAST fact about a check that already ran and carries no forecast (the C1 rule: distance to a scoring tier is a fact, distance to a TRADE is a forecast that never ships). | spec §1.4, §1.5; seed §3; D-79, D-103 | **Ruled Sep 8**; the cap recorded Sep 9 at build. Server: `composeCandidatesStamp`. Client: `selectFlagged` in `src/screens/battleView/selectBench.js` + `PaneBench.jsx`. |
| **D-113** | **One server flag for the three stamps; CLIENTS RENDER ON PRESENCE; the flip is the smoke.** `TICK_STAMPS_ENABLED` ships false and no reader imports it: every surface renders when `evaluation.heard` / `.evidence` / `.vintages` / `.candidates` exist. Pre-flip entries, every `budget_skipped` entry and any entry whose prompt build threw carry none of them, so every client is byte-identical until the founder's own flip PR. | spec §1.1, §4; seed §8 | **Ruled Sep 8.** Built: the flag at `src/config/featureFlags.js:2237` (pinned, `DARK_BY_DESIGN`), the one splice in `api/cron/agent-evaluate.js`. Every client surface asserts a byte-identical render with no stamp (§1–§4's own rows). |
| **D-114** | **The typed directive shares the chip route's transaction:** latest-wins with the actual replaced thread recorded; the pre-call check kept, the charge in-transaction, an eleventh commits with `overBudget`; the error body attests `persisted` / `charged`. | spec §2; P-1a/b/c; D-104, D-105 | **Ruled Sep 8. NOT BUILT** — B2, after its own review. |
| **D-115** | **The Direct menu's home is the Chat section beside the `Files:` chips** (amends D-53's header placement); not built in Phase B. | spec §5; D-53 | **Ruled Sep 8. NOT BUILT** — recorded for its own phase. |
