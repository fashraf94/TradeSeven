# Call Record — Field Contract V1.4

**Date:** 2026-09-23 · **Author:** Fable (Command Center arc) · **Supersedes:** V1.3 · **Status:** **amendment for blessing (Sep 24, second cut)** — two clarifications ruled by the founder during Build 0 and adjudicated in Astra's branch review of PR #903 (BR-1, BR-4): the origin rule (§4) and the `next_check` judgment boundary (§5, §6, §9); the second cut applies Astra's C1 (unconfirmed ≠ uncommitted) and the §6 precedence clarification from `docs/audits/20260924_CONTRACT_V1_4_REVIEW.md`. No field shape changes. The pilot's blessed header pins this file's SHA-256 as computed from the committed bytes at the docs commit. **Consumers:** cockpit spec V1.1 (to be written against this cut) and the pilot spec.

**Changelog from V1.3 (Build 0 rulings A-1 and E-3, adjudicated in PR #903's branch review).** H1 origin: an absent or null frozen watchlist is `agent_initiative` regardless of any config hash; `provenance_unresolved` only for a present-but-unusable snapshot; the reason `hash_without_watchlist` is retired (§4). H2 `next_check`: `expiresAt` is the judgment-eligibility boundary, not a hard expiry — the first check that reaches the still-open call at or after it judges it once (§5, §6); a judgment is a committed transaction; Build 1's sweep must honor that opportunity (§9).
...
| `hypothesisRef` | versioned \| legacy \| null | **content, not chronology:** versioned `{ watchlistId, hypothesisVersion }` when the frozen battle context carries a validated version; legacy `{ watchlistId, equippedConfigHash }` whenever the frozen context (`agentContext.equippedWatchlist`, `resolvedAgentManifest.equippedConfigHash`) has equipped provenance without a version, regardless of when the battle was created; **The complete origin rule (H1):** `equipped` carries a validated versioned ref or a legacy ref; `agent_initiative` carries `null` because no equipped origin exists — **that is, the frozen `agentContext.equippedWatchlist` is absent or `null`, regardless of whether a config hash exists** (every battle carries a config fingerprint; a hash alone is never watchlist provenance); `provenance_unresolved` carries `null` plus `provenanceReason` **only when a watchlist snapshot is present but unusable** (`snapshot_corrupt`; `config_hash_missing` for a valid snapshot with neither a version nor a hash). **Readers never infer origin from a null ref alone** — `origin` is the authority, the ref is its payload. A missing version still selects legacy whenever valid legacy provenance exists; a missing version never becomes null. **`battleId` is omitted inside this battle-scoped record; any ref exported outside a battle-scoped record carries `battleId` beside it.** A frozen equipped watchlist never becomes initiative because its version is absent; a config hash alone is not watchlist provenance; the writer never fetches the live watchlist to complete a frozen snapshot. |
| `origin` | `equipped` · `agent_initiative` · `provenance_unresolved` | the authority for provenance (see `hypothesisRef`); `provenance_unresolved` carries `provenanceReason` only when a watchlist snapshot is present but unusable (`snapshot_corrupt`, `config_hash_missing`); visible and conservative, never read as initiative; a hash without a watchlist is `agent_initiative`, not unresolved (H1) |
...
|---|---|
| `next_check` | the first **eligible evaluator slot** strictly after `promptBuiltAt` — from the registered cron schedule intersected with the calendar's regular-session bounds, pinned against `vercel.json` for cadence drift; never `lastCheckedAt + interval`. **For this basis `expiresAt` is the judgment-eligibility boundary, not a hard expiry (H2):** the first check that reaches the still-open call with an observation at or after `expiresAt` judges it once — `hit` if the condition is met at that observation, else `expired_unresolved` — state and receipt in one transaction. A judgment exists only when that transaction commits. An attempt that does not commit — failed, skipped or deadline-cut before its write — has judged nothing; an **unconfirmed** attempt has an unknown outcome and may commit late, so a later transaction judges from its own observation **only if its fresh read still finds the call open**. A late first reach may therefore judge after the slot while the parent is active; its receipt is that check's own observation, never a quote from the missed slot. Racing scans guarantee one committed judgment, not that the earliest observation wins. No battle-level clock takes part. |
...
`open` → `hit` · `invalidated` · `expired_unresolved` · `ended_with_battle`. Terminal states are honest outcomes. For every basis except `next_check`, an observation after `expiresAt` expires the call; for `next_check`, §5 (H2) governs — the first committed judgment at or after `expiresAt` decides `hit` or `expired_unresolved`.

**`hit` requires an encounter observation:** the check actually examined the symbol with a finite positive quote at an instant inside the horizon, from a named evidence source — never the global quote map or a stored price. No usable observation → no `hit`. Above/below is `>` / `<` against the checked quote at the observation instant. Expiry wins when the observation is outside the horizon, **subject to the `next_check` judgment rule in §5 (H2)**; completion wins for a terminal parent. **Eligible pre-slot hits are unchanged for every basis, `next_check` included.** Non-model checks observe with their own instant, never a fabricated `promptBuiltAt`.
...
Dependencies: the deferred-beat PR's run-document contract (`agentEvalRuns`) for the sweep; the sweep queue in its own server-only collection (`callSweepQueue/{battleId}`, armed when a call is created — no new top-level battle key); **the Build 1 sweep worker must honor the `next_check` judgment opportunity (H2): it may expire a `next_check` call only under a separately agreed unobserved-expiry policy, never mechanically at its slot — that would recreate E-3; other bases sweep on `expiresAt` as before;** the tick-side `calls[]` reference (C-3 allowlist by path, §2-reviewed); `firestore.rules` for `declarations` and `calls`. Retired by contract: the threshold-lint flip; the drop receipt.
