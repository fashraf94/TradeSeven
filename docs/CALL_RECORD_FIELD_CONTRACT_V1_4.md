...
**Date:** 2026-09-23 · **Author:** Fable (Command Center arc) · **Supersedes:** V1.3 · **Status:** **amendment for blessing (Sep 24)** — two clarifications ruled by the founder during Build 0 and adjudicated in Astra's branch review of PR #903 (BR-1, BR-4): the origin rule (§4) and the `next_check` judgment boundary (§5, §6, §9). No field shape changes. The pilot's blessed header pins this file's SHA-256 as computed from the committed bytes at the docs commit. **Consumers:** cockpit spec V1.1 (to be written against this cut) and the pilot spec.
