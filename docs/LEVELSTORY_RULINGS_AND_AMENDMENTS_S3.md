...
| S3-C6 | Composite availability (clusters & snapshots) | max of members' formation/firstKnown/firstTradable | §5.3 defines availability per source structure; a cluster/snapshot is a composite. Latest-member is the conservative direction — a composite is never available earlier than its newest constituent. **Standing usage rule (review-flagged):** the snapshot-level triple is an availability BOUND for the reference rule only; because daily-pivot members stamp `D`, any snapshot containing one reads firstKnown=firstTradable=D. Age/freshness-like features (S4+) must derive from MEMBER triples or family `bornDate`, never from the composite triple. | Slightly late availability for old zones whose newest touch is recent; never lookahead. Misuse risk documented, not structural. |
...
| S3-C14 | Split needs ≥2 matched snapshots — **for counting AND execution** (review-hardened) | yes | Confluence chaining can produce a single snapshot whose members span >1.5%; a single snapshot cannot be partitioned. Originally the counter ran on single-snapshot spans with execution deferred; the mandatory S3 code review demonstrated that lets unrelated accumulation fire a permanent split off a one-day transient. Now each of the 5 consecutive counter sessions itself requires ≥2 matched snapshots AND >1.5% span — the evidence counted is exactly the evidence executed on. | A wide single chained snapshot never contributes to split evidence; conservative. |
