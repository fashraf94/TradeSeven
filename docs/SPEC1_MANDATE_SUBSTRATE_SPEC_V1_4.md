...
**Fork ledger — season risk math → `mandateRiskMetrics.js` (realized Phase 3, 2026-08-12).** The remaining declared fork is now concrete. `mandateRiskMetrics.js` re-implements the §4.2 metric family against the `seasonLeaderboard.js` contract Q4 verified (fork is a rewrite, not a call into dormant season code). **Carried:** the null-on-degenerate posture (Sharpe null below 2 samples / degenerate stddev — hardened from the season's exact `sd === 0` to a relative-epsilon test, since identical-return series carry ~1e-18 float noise that would otherwise print an astronomically large Sharpe, the precise flattering number §4.2 forbids), the running-max drawdown shape (negative-percentage convention), and the `normalizeMetric` composite normalization ranges. **Deliberately EXCLUDED (Q4 hazards, §4.1):** season `computeTradeStats` (proceeds-based "dollar P&L") is not ported — no trade-stat metric is derived; and the season's week-boundary recovery factor is replaced by period-return ÷ |max drawdown| with the §4.2 zero-drawdown→null rule. **P3 additions with no season analogue:** warmup minimums (`MANDATE_METRIC_MIN_ROWS`), the I6 partial-row discipline (variance exclusion + `degradedMarks`), FR-2 lens separation over row `quarterIndex` tags, and composite weight renormalization over the non-null subset with contributors recorded. Friction also went live this phase: `MANDATE_FRICTION_MODEL_VERSION = 'p3_cap_tier_v1'` (market-cap-tier slippage + spread proxy, §4.1), receipts' honesty labels unchanged.

**Prompt-input registry (Phase 2 reading, FOUNDER-RATIFIED 2026-08-12 close-out).** §3.2's "register new prompt-contributing modules in `PROMPT_CONTRIBUTING_MODULES` same commit" is satisfied by a **mandate-scoped** registry (`__fixtures__/mandatePromptRegistry.js` + `mandatePromptAssembly.honesty.test.js`) rather than the battle-specific C-20 registry — the house rule's intent is "no unregistered prompt contributor, enforced by tripwire," and a mandate-scoped registry meets it directly. Literal membership in the C-20 list is **explicitly declined**: its forbidden-signal list and fenced-assembler tripwire are battle-only and would cross-couple two suites whose signal lists have nothing to do with each other. The mandate tripwire enforces the substantive §3.2 guard — the assembler imports **no** live registry/model-config source (identity comes only from the pinned vintage) and every prose module is classified.
