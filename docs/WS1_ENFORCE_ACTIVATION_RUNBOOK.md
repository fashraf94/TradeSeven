...
2. **Orphan-coverage check (raised at Phase 2):** the cleanup core mirrors the projection's reach — it evaluates overrides only for ruleIds **listed** on a bundle (`api/_utils/ruleCompatCleanup.js:67`). With the client-side prune retired (Phase 2), a hard override orphaned by a rule-removal would survive cleanup and could resurrect via re-add under enforce (add-time writes only `ruleIds`, no gate). **Today this is moot — no authored overrides exist in production (the authoring door has never been open)** — but confirm the dry-run census shows zero `ruleHardness` entries at all; if any exist, extend the cleanup to orphans before `--live`.
