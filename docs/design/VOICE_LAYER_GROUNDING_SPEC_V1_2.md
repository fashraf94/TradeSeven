# Voice-Layer Grounding — Spec V1.2 (Sol confirm pass applied)

**Date:** September 7, 2026
**Status:** Spec V1.2, pre-build. **Supersedes V1.1.** Sol's confirm pass (Sep 7: 0 BLOCKER, 5 MAJOR, 3 governance; verdict PROCEED WITH CORRECTIONS) is applied in full — Appendix C. **Build-ready on the founder's confirmation of §6.4.**
...
**Suggested commit location:** `docs/design/VOICE_LAYER_GROUNDING_SPEC_V1_2.md`
...
```
- **Verbatim means bytes.** No stripping in the prompt; the model reads `**`. The test is byte equality against the stored field.
...
- `'shadow'`: assembles the new prompt and logs both to the shadow stream; **sends the old**. Proves assembly and token size on real battles; nothing else.
