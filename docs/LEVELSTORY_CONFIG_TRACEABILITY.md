...
| `honesty.acceptance.minUniqueDates` | `15` | **S5-A2** (pre-registration amendment, pre-outcome; effective n = unique-date count under date-clustered bootstrap) |
| `primaryQuestions.P4` | F1 vs F2 (`compare`, `f3Disposition`) | **S5-A1** (pre-registration amendment, pre-outcome; F3 cell structurally dead — pooled into F2+ gates, exploratory footnote) |
| `features.fingerprint.rvolApproachBuckets` | LOW <0.8 / MID 0.8–1.5 / HIGH ≥1.5 | **S5-C1** (P3's three buckets, fixed edges pre-registered before outcomes) |
...
*Session-4.1 update (2026-07-12): `STUDY_CONFIG_VERSION = 3` (unchanged). Episode geometry unit correction — the S4 thresholds were stored 4× too tight (ATR values read as `u`-multiples without conversion). Corrected + renamed off `*Atr`: `episode.zoneHalfWidthU 1.0` (=0.25·ATR), `episode.closeSeparationU 4.0` (=1.0·ATR), `episode.crossLevelDedup.dedupIntersectU 2.0` (=0.5·ATR). Role-flip threshold (0.5·u) decoupled and unchanged (role machine untouched). Regression guard `tests/25`. Amendment 8 — see `docs/LEVELSTORY_RULINGS_AND_AMENDMENTS_S4.md` and `docs/discovery/SESSION4_1_GEOMETRY_CORRECTION_REPORT.md`.*
*Session-5 update (2026-07-12): `STUDY_CONFIG_VERSION = 3` (unchanged). Feature layer built (`04-features.js`, `lib/features*.js`): 63 availability-classed features (60 pre_touch / 3 post_touch), manifest-closed (`lib/features.js:FEATURE_MANIFEST`). Pre-registration amendments S5-A1 (P4 → F1 vs F2) and S5-A2 (`minUniqueDates: 15`) + S5-C1 RVOL buckets — the LAST legitimate pre-registration changes (freeze rule: frozen forever once S6 computes an outcome). Choice register S5-C1…C17 — see `docs/LEVELSTORY_RULINGS_AND_AMENDMENTS_S5.md`.*
