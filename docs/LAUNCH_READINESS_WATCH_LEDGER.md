...
### G2 — Agent single-battle lock: a vs-CPU deploy during a pre-battle slot pod blocks the pod's Monday agent deploy
...
- **Open severity question (resolve before launch):** is this a **Day-1 gap** or a **whole-week absence**? The Tue–Fri redeploy targets *incumbents* (`buildIncumbentSeats`, `tournamentOrchestrator.js:687`) — agents already in the group's battle. A Monday-*skipped* agent may not qualify as an incumbent and may never be picked up for the rest of that pod's five-day battle. Confirm whether the daily redeploy recovers a never-deployed agent; if not, the agent is out for the whole pod.
- **Fix options (both need founder sign-off):** (a) **League takes precedence** — `runPrescribedTournamentDeploy` completes/overrides an active *non-tournament* battle before deploying; this edits the **calibration-fenced** `api/agent/decide.js` and is allowed only via the sanctioned fence entry (BUILD_RULES §1). (b) **Block the vs-CPU deploy** at the deploy entry (client + server) for a user holding a pre-battle slot pod, so the League commitment always wins. Decide which before `LEAGUE_LIVE_DRAFT` goes live with real users.
- **Origin:** found during the Seated Waiting Room build (2026-07-19) — see `docs/audits/20260719_SEATED_WAITING_ROOM_PHASE0_DISCOVERY.md` §12.
