...
2. **Claim-window enforcement** — server-authoritative: `place-claim.js:65-70` (403 `window_closed`), day-5 cutoff `:101-102`, pending cap `:129` (advisory; parallel submissions can both land — resolution still honors it). Flip cap server-enforced (`flip.js:150`). Client mirror is display-only (parity-locked).
...
5. **Known best-effort gaps** — the eliminated-interstitial localStorage pointer (`roundBoundaryAck.js`) is best-effort by design; the advisory claim-cap race. Both documented, neither a leak.
