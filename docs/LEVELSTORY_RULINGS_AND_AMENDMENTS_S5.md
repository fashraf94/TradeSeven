...
| S5-C21 | RVOL volume basis | cross-session volume comparisons use the S3-C1 split-adjusted basis (volume ÷ adjFactor) — raw share counts are incomparable across a split (NVDA 10:1 is inside the window; raw comparison inflated rvol up to ~10× post-split; review fix, regression-tested) | `features-intraday.js:cumVolBefore/rvolApproach`; `tests/27` |
| S5-C22 | earnings coverage boundary | reports predating the daily series are dropped, never aliased to a phantom session-0 report (which fabricated `sessions_since_last_earnings` and polluted the median gap); truncated peer prior-5d windows are null, not 0-rates; primary-origin lookback is exactly 252 candidates (review fixes) | `features-daily.js`; `features-market.js`; `tests/29` |
