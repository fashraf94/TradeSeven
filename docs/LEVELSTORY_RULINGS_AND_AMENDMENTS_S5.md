...
| S5-C14 | base definition | ≥10 consecutive sessions inside a 2.5·ATR(run-start) band, occurring after the leg advanced ≥3 ATR; non-overlapping greedy count | `features-daily.js:baseCountAt` |
| S5-C15 | momo meter details | basket perf measured rel-SPY from formation (T−21) to D−1; sector-neutral via in-universe sector demeaning (no ETF dependency); slope compare carries a 1e-9 float epsilon | `features-market.js` |
