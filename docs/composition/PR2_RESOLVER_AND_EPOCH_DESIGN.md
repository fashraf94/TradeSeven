...
| **Background loops / provisioners** (`trainingClone.provisionTrainingClones`, `casualClone.ensureCasualClone` *(PR #716, merged mid-PR-2 — endpoint-invoked raw-write provisioner: births + re-syncs loadout AND the rules/bundles subcollections)*, `seedArchetypeTraitsDeterministic` via its callers, `rule-compat-cleanup --live`, `mastery-preflip-normalize --apply`, `ws1-observe-walk`) | `await assertWriteEpochOpen(db)` at entry **and per batch/agent iteration** (bounded conformance — a loop straddling the close stops at the next iteration boundary); the casual-clone endpoint maps `epoch_closed` → 409 like the fenced endpoints; PLUS enumerated in the §8 runbook's pause list (belt and suspenders). A46 additionally ratchets on `copyAgentSubcollections` callers so the raw-write clone class can never slip the census again | flag-gated no-op; scripts additionally listed in the census with their pause step |
| **`decide.js` activeRules write** (`:279-281`, fenced file) | **Classified DERIVED, transitively fenced — FOR DARK OPERATION ONLY (Sol adjudication, Aug 6, 2026).** `projectActiveRules` is a pure function of rules docs + non-archived bundles (status/ruleIds/ruleHardness/name) + `agent.equippedTraits` — ALL epoch-fenced stores above — with the written value passing through `resolveForDeploy` (pure, removal-only) — so a window write can only re-derive values the frozen inputs already imply; it cannot commit new identity content. The residual scan reads the **authority stores through the resolver**, never `activeRules` (census host A: "derived — not a primary target"). The write is skipped entirely mid-battle. **The transitive argument holds WITHIN one epoch only — it does not survive an activation boundary** (Sol's cross-generation counterexample, ruling block below), so the **PR-4 fenced splice is REQUIRED**: epoch/generation validation before the write + a generation stamp on the written projection; readers reject stale-generation projections. |
...
> **RULING HISTORY (both recorded):**
> 1. *Aug 6, 2026, D1 ratification message:* DERIVED classification ACCEPTED; splice optional.
> 2. *Aug 6, 2026, Sol design-review adjudication — REVERSES (1):* the DERIVED classification stands **for dark operation only**. **The PR-4 fenced splice is REQUIRED** — epoch/generation validation before the `activeRules` write + a generation stamp; readers reject stale-generation projections. **Recorded reason — Sol's cross-generation counterexample:** the transitive argument freezes the projection's inputs *within* an epoch; a decide.js window in flight across the activation boundary reads the authority stores at generation N−1 and commits its re-derived projection *after* generation N activates — persisting an old-identity projection past the watermark, unmarked and indistinguishable from a current one. Ruling of record + closing acceptance test: `docs/composition/ACTIVATION_PRECONDITIONS.md`.

**§8 runbook — STEP 0 (founder adjudication, Aug 6, 2026):** before any epoch close or activation step, the runbook's first named item is a **census re-run at the exact merged SHA** — the full A46 suite (plus the PR-3 B3 protected-store scan once it exists) executed at the SHA actually deployed, with SHA + result recorded in the runbook log. Writers merged between the census's last green run and the deploy SHA (the PR #716 `casualClone` interception is the precedent) are caught before the fence goes live, not after.
...
See `docs/composition/PR2_FLAG_OWNERSHIP.md` for the table of record. Summary: `COMPOSITION_ENFORCEMENT_MODE='off'` ('off'|'observe'|'enforce' — offer/equip + whole-config boundaries), `COMPOSITION_EPOCH_FENCE_ENABLED=false`, `COMPOSITION_MIGRATION_FEED_ENABLED=false` (A44) — all three SERVER flags in `api/_utils/compositionConfig.js` (build finding: the masteryConfig.js precedent — 8 endpoint suites mock `featureFlags.js` with explicit export lists, so server flags there break their mocks); `COMPOSITION_DISPLAY_ENABLED=false` (D2, client) in `src/config/featureFlags.js`. Every row carries a byte-identical-while-dark test (A23). The rules-layer fence is dark by construction (absent doc = open) and inert until Console deploy.
