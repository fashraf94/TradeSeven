...
| **`decide.js` activeRules write** (`:279-281`, fenced file) | **Classified DERIVED, transitively fenced — not an authority-store writer.** `projectActiveRules` is a pure function of rules docs + non-archived bundles (status/ruleIds/ruleHardness/name) + `agent.equippedTraits` — ALL epoch-fenced stores above — with the written value passing through `resolveForDeploy` (pure, removal-only) — so a window write can only re-derive values the frozen inputs already imply; it cannot commit new identity content. The residual scan reads the **authority stores through the resolver**, never `activeRules` (census host A: "derived — not a primary target"). The write is skipped entirely mid-battle. *Optional belt-and-suspenders:* a one-line fenced splice at the PR-4 sanctioned fence entry if the founder wants literal coverage; **not** required for soundness and not done in PR 2. |
