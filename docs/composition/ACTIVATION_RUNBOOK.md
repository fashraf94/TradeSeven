...
1. **Quiesce the two live protected-store writers that are not runbook steps** (founder rulings, Aug 12 — the casual-clone ruling and R7):
   ...
   **NOT removed, and why (the same audit, recorded so the next reader does not re-derive it):** `live-draft-fire` (`*/10 * * * *`) and `tournament-orchestrator` (`*/10 11,12,13,14,21,22,23 * * 1-5`) both reach `ensureCpuAgents` (`tournamentCpu.js:111`, via `liveDraftLifecycle.js:201` and `tournamentAdvancement.js:797`), which **creates `agents/cpu-*` docs** with no epoch guard. They are left scheduled because pausing them stops live drafts and Friday round advancement — a product decision, not a sweep decision. The write is a lazy get-or-create (idempotent by deterministic doc id, existing docs never rewritten), so it lands in the sweep **only if a group forms or advances inside the window**. If the window spans an active live draft or a Friday-evening advancement, either remove these two as well or attribute the resulting `agents/cpu-*` creations explicitly in the step-1.11 sweep. `agent-evaluate` needs no removal: its `agents/*` write is `decide.js`'s `activeRules` projection, censused DERIVED and carrying the PR-4 fenced splice's epoch/generation validation. No cron writes `compiledBuilds` — `writeCompiledBuildsInTx` has 11 call sites and every one is a fenced endpoint.
...
**MERGE FREEZE — declared 2026-08-12 22:57 -0500 (founder).** No merges to `main` until activation completes or the founder lifts it. Rationale: `main` took two feature merges (PR #750 Mandate Phase 3 at 21:04:27, PR #751 flag-off at 21:34:57) inside the pre-flight window, each re-opening the step-0 preflight and one of them changing `firestore.rules`. Step 1.2 pins THE ACTIVATION SHA and forbids any later commit; the freeze is what makes that pin hold. Freeze SHA: `2acc6189611f2207efaf4d1a3d64b0b99ae3c3dc` — **re-pinned 2026-08-13** from `3069ba09ab2fe301810530a4620b9a032d9b1f64` after PR #753 (the docs-only runbook cleanup), permitted under this freeze's activation-runbook exception. **Every merge taken under a freeze exception re-pins this value**, so the pin of record is the current `main` tip at the moment step 1.2 runs, and this line is updated in the same PR as the exception merge. Lift is a logged act with its own timestamp + operator.
