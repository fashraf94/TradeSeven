...
14. **The DR-13 identity block is LIVE — flipped ON in the founder flag-flip PR (DR-13 endgame).** `EVAL_IDENTITY_BLOCK_ENABLED = true` (`src/config/featureFlags.js:1170`); `renderEvalIdentityBlock` (`evalIdentityBlocks.js:155-156`) now renders the block for every canonical key (unknown/undefined keys still omit + log — the flag-independent rule). The flag header states the ON contract: each eval tick's system prompt carries the deciding archetype's identity block. Guide §7 calls the identity block "the only carrier of quality-floor language into swap decisions" — **it now carries that language**, including the `guardian` block's "low-beta required" text. Flip evidence: 840 paired decisions (temp 0 + 0.4) showed zero decision drift, so the block ships identity-consistent *reasoning* (conviction + rationale framing), not measurable decision change at current gate settings; the off-state remains the deliberate-revert path.
