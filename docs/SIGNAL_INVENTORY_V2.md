...
### 2B. Fundamental family — PARTIALLY WIRED as of the Fundamental Wire arc (Jul 25 2026)

All rows live on **`peerRankings/{ticker}`** (cron `0 11 * * 1-5`). **No agent path reads that collection directly** — the statement below is unchanged and remains true.

**What changed (founder rulings D1–D7, Jul 25 2026).** The Fundamental Wire arc added a **selective mirror**: `buildFundamentalsMirror` (`compute-index-intelligence.js:432-517`) copies a **minimal** field set off the peerRankings doc this cron already holds in memory onto each `indexIntelligence/stockRankings` stock entry — the document **both** agent paths already load — and `api/_utils/fundamentalsRender.js` renders it into the eval live-context block and the draft/board CSV. Gated on `FUNDAMENTAL_MIRROR_ENABLED`; **a row is only wired when that flag is true.**

Three constraints govern the statuses below and must be read with them:

1. **The mirror is deliberately minimal (ruling D3).** It carries only what the six un-hidden rules need. Metrics whose rules were re-triaged to `hidden_absent_substrate` (ruling D1) are **not** mirrored — SIG-022, SIG-023 and SIG-042 therefore stay **unwired**, by design, not by omission.
2. **`beatRate` is mirrored REAL-ONLY (ruling D2).** `compute-rankings.js` now stamps `metrics.beatRateSource` = `'computed' | 'sector_default'`; the mirror omits `beatRate` entirely when the value is the `SECTOR_BEAT_RATES` fill, so a sector constant can never render as a per-company fact.
3. **Vintage travels with the data.** Each mirrored entry carries the source doc's `computedAt`, and the render marks entries older than the block's newest vintage — the per-ticker drop-out and dead-producer-run cases that previously left stale values indistinguishable from fresh ones.

| Producer | Schedule | Feeds |
|---|---|---|
| `compute-index-intelligence` (mirror leg) | `30 10,11` + `0 14-20 * * 1-5` | the wired subset below, onto `indexIntelligence/stockRankings` |

| verificationId | Kernel category | Verified binding | Status |
|---|---|---|---|
| SIG-021 | valuation ratios | `metrics.trailingPE` (`compute-rankings.js:629`; clamped <0 or >500 → null `:630-632`; written `:1352`) · `priceBookMRQ` (`:637`, ≤0 → null; `:1354`) · `evEbitda` `:1351` · `priceSalesTTM` `:1353`. ⚠ Field is **`priceBookMRQ`**. **`forwardPE` ABSENT** (dead UI reader only). **WIRE:** `trailingPE` (with its sector median) and `priceBookMRQ` mirrored + rendered — they serve `fund-value-pe` and `fund-bank-pb`. **`evEbitda` and `priceSalesTTM` are NOT mirrored** (their only claimant, `f-10`, needs a sector→metric routing table that does not exist — re-triaged). | **VERIFIED (partially wired — P/E + P/B only)** |
| SIG-022 | leverage / solvency | `debtToEquity` — genuinely computed `(shortLongTermDebtTotal ?? longTermDebt) / totalStockholderEquity` (`compute-rankings.js:434-437`), no-debt→0 (`:438-439`), written `:1363`. Siblings `currentRatio` `:1364`, `interestCoverage` `:1365`, `netDebtEbitda` `:1366`. ⚠ From the most recent **YEARLY** balance sheet (`:429-431`) — the real staleness bound is the last annual filing. **WIRE: NOT MIRRORED, deliberately.** Both claimants were re-triaged to `hidden_absent_substrate`: `f-09` names a sector **mean** where only a **median** is computed, and `fund-financial-health` needs a strong/moderate rating band that exists nowhere. Mirroring the metric without those would not make either rule evaluable. | **VERIFIED (unwired — by design)** |
| SIG-023 | cash generation | `fcfYield` = `(fcfTTM / marketCap) × 100` (`:649-650`, written `:1355`); `fcfMargin` `:1357`. ⚠ **Absolute FCF not persisted**; explicitly `null` in the season context (`seasonEvalContext.js:120`). **WIRE: NOT MIRRORED, deliberately.** Its only claimant `f-08` asks for a **universe-scoped** percentile; every fundamental percentile here is **sector-scoped** (`rankSectorStocks:780`), so the rule was re-triaged rather than served with the wrong population. | **VERIFIED (unwired — by design)** |
| SIG-024 | growth | `revenueGrowthYOY` `:1344` (EODHD `QuarterlyRevenueGrowthYOY`, `marketDataCache.js:292`) · `earningsGrowthYOY` `:1345`. **Plus forward consensus growth**: `estimatesCache/latest.stocks.<t>.forwardEstimates.{currentYear,nextYear}.growth` (`compute-estimates.js:265`, assembled `:311-316`, persisted `:458`). **WIRE:** `revenueGrowthYOY` mirrored as **`revenueGrowthPct` — ×100 normalized at write**, because the source is a **FRACTION** and `fund-revenue-growth`'s threshold is stated in percent (unfixed, the rule silently inverts). `earningsGrowthYOY` and forward consensus are **NOT mirrored** (no un-hidden rule claims them). | **VERIFIED (partially wired — revenue growth only)** |
| SIG-025 | earnings surprise history | `beatRate` `:1368` · `avgSurpriseMag` `:1369` · `surpriseConsistency` `:1370` · `avgEarningsSurprise` `:1373`. ⚠ **Four inconsistent surprise definitions exist**, two inside one function: `compute-rankings.js:527-533` uses mean-of-**ABSOLUTE** for 2–3 usable quarters and mean-of-**POSITIVE** (`:548-551`) for ≥4. No decile. **WIRE:** `beatRate` mirrored **REAL-ONLY** — the new `metrics.beatRateSource` marker (`:506-541`, ruling D2) suppresses the `SECTOR_BEAT_RATES` fill so a sector constant never renders per-company. `avgSurpriseMag` reaches the agent only as a **sector percentile** (SIG-026), never as a raw value. `surpriseConsistency` / `avgEarningsSurprise` NOT mirrored. ⚠ **The definition inconsistency above is NOT resolved by the wire** — it is inherited and tracked. | **VERIFIED (partially wired, definition-inconsistent)** |
| SIG-026 | sector-relative valuation | **`pillars.<p>.dimensions.<d>.{value, rank, percentile, sectorMedian}`** — `compute-rankings.js:1309-1329`; medians computed in `rankSectorStocks` (`:902-912`), landed `:1320`. **This is the sector-average answer: medians + within-sector percentile per ranked dimension.** **WIRE: two pairs only** — `valuation.dimensions.trailingPE.sectorMedian` (serving `fund-value-pe`'s default "vs sector median" predicate) and `earningsConsistency.dimensions.avgSurpriseMag.percentile` (serving `f-07`'s decile leg). **Every other dimension pair remains unwired.** ⚠ Still a **MEDIAN**, still **SECTOR**-scoped — any future rule naming a *mean* or a *universe* population is not served by this. | **VERIFIED (partially wired — 2 of N dimension pairs)** |
| SIG-027 | analyst consensus trend | `earningsRevisions` `:1372`; upstream `computeRSR` (`compute-estimates.js:210-215`), `computeEMS` (`:217-222`), `epsRevisionsUp/DownLast30days` (`:303-304`), sector revision diffusion `(up−down)/total` (`:324-349`), `emsPercentile` (**within-sector**, `:358-382`). **WIRE:** `earningsRevisions` mirrored as **`earningsRevisions30d`** — named for its hard-coded 30-day window (`epsTrend30daysAgo`, `:381`), which is the only window the source computes. It serves `f-12` **at that rule's default setting only**; the other selectable `{days}` values (14/21/28/35/42/49/56) remain unserved. The upstream `compute-estimates` fields are NOT mirrored. | **VERIFIED (wired at 30d)** |
| SIG-028 | market capitalization | `metrics.marketCap` (`compute-rankings.js:643,719,1377`). Reaches the **dormant** season context (`seasonEvalContext.js:121`). ⚠ **Raw value only — no classification buckets.** **WIRE:** the mirror now **derives** `marketCapClass` ∈ `large \| mid \| small` at write (`>$10B` / `$2–10B` / `<$2B`, per `fund-market-cap`'s own param labels) and renders it. **This closes the "Market-cap classification — REFUTED AS NAMED" gap in §3A**: the classification is now bound to a doc on an agent path. The **raw value is still not mirrored** — only the bucket. | **VERIFIED (wired as classification)** |
...
| **Market-cap classification** | ~~**REFUTED AS NAMED**~~ → **RESOLVED (Jul 25 2026, Fundamental Wire)** | *Was:* raw `marketCap` persisted (SIG-028), **no bucket field on any doc** — bucketing existed only as an ephemeral earnings-queue heuristic (`api/earnings/sync-queue.js:339-341`) and a client service (`src/services/recommendationEngine.js:188-202`), neither persisted nor on an agent path. *Now:* the mirror derives `fundamentals.marketCapClass` at write (`compute-index-intelligence.js:497-499`) onto `indexIntelligence/stockRankings` and renders it (`MCAP_CLS` in the draft CSV, `mcap=` in the eval block). **The classification is now bound to a doc on an agent path.** Authorable. |
...
| **Sector peer ranking (fundamental)** | **PARTIALLY REFUTED** (narrowed Jul 25 2026) | Two disconnected axes: technical (`sectorTechnicalRank`, SIG-012, agent-visible) and fundamental (`sectorMedian` + within-sector percentile, SIG-026). The fundamental axis is **no longer wholly unwired** — the Fundamental Wire mirrors exactly **two** dimension pairs (`trailingPE.sectorMedian`, `avgSurpriseMag.percentile`). The axes remain **disconnected**, and every other fundamental dimension pair is still unwired, so the verdict stands as *partially* refuted. |
...
| **Fundamental** | ⚠ **PARTIALLY UNBLOCKED** (Jul 25 2026 — the mirror landed) | The wire arrived, and it revealed the original verdict was **half right**. *Wired and citable:* P/E + its sector median, P/B, revenue growth (percent-normalized), market-cap class, 30-day EPS revisions, real-only beat rate, surprise-magnitude percentile — six rules un-hidden (`fund-value-pe`, `f-07`, `fund-revenue-growth`, `fund-bank-pb`, `fund-market-cap`, `f-12`) plus `r-07` on the industry render line. *Still blocked, and NOT a wiring gap:* six rules were re-triaged to `hidden_absent_substrate` because each names a statistic **no producer computes** — universe percentile (`f-08`), sector **mean** vs the median that exists (`f-09`), prior-period growth for a second derivative (`f-11`), a sector→metric routing table (`f-10`), a health rating band (`fund-financial-health`), a per-quarter surprise-sign series (`fund-earnings-surprise`). **"One wire away" held for half the family; the other half was a computation gap all along.** |
...
| `compute-rankings` | `0 11 * * 1-5` | SIG-021…028, 042 (`peerRankings`) — plus `metrics.beatRateSource`, the D2 fabrication marker |
