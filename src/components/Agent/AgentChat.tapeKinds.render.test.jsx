...
describe('D-84 — four kinds in one stream', () => {
  ...
  it('`Bench note` is the BENCH\'s word; a `potential_exit` note is a `Holding note` (review L1-F1; grounding spec §5)', () => {
    ...
    const anticipation = (direction, over = {}) => render({
      chatExchanges: [{
        userMessage: null,
    ...
    // — for a note produced UNDER THE GROUNDING CONTRACT (the marker). A
    // legacy, model-written exit note keeps no eyebrow, exactly as shipped
    // (review R-02): the word never lands on a forecast the retired prompt wrote.
    const exitNote = anticipation('potential_exit', { groundingVersion: 1 });
    ...
    const legacyExit = anticipation('potential_exit');
    expect(legacyExit).toContain('if it loses the 20-day');
    expect(legacyExit).not.toContain('data-tape-kind-eyebrow');
    // The bench word is not gated: it was ruled before the marker existed.
    expect(anticipation('potential_entry', { groundingVersion: 1 })).toContain('data-tape-kind-eyebrow="Bench note"');
