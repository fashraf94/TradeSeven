...
const DARK_BY_DESIGN = {
  ...
  EVAL_DEFERRED_BEAT_ENABLED:
    'Eval-cron instrumentation D3 (docs/audits/20260923_BUILD_EVAL_DEFERRED_BEAT.md) — the `check_deferred` status-feed beat for each battle the evaluation loop did not reach (cap 25 per run, ≥ 5 s of the hard ceiling left) and its tape line. Built dark; flips in the founder\'s own one-line PR, never a build PR, after the flip prerequisites the build report lists (the other status-feed readers). Crons do not run on preview: the first beat is seen only in production. The run document agentEvalRuns/{runId} is NOT behind this flag — it is always on',
