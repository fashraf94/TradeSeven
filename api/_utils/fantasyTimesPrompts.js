...
SECTOR PERFORMANCE (today's ETF changes — use this to verify sector claims):
${ctx.sectorSnapshot.map(s => `  ${s.sector} (${s.etf}): ${s.changePercent > 0 ? '+' : ''}${s.changePercent.toFixed(2)}%`).join('\n')}
