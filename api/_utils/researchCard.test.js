...
const QUOTE = { current: 151.27, timestamp: 1788962400, candleDate: '2026-09-08' };
...
describe('the quote instant — the unit trap', () => {
  it('reads EODHD’s UNIX SECONDS as seconds, not as milliseconds in 1970', () => {
    expect(quoteInstant(1788962400)).toBe('2026-09-09T14:00:00.000Z');
    expect(new Date(quoteInstant(1788962400)).getUTCFullYear()).toBe(2026);
  });

  it('still accepts a millisecond instant, and refuses everything else', () => {
    expect(quoteInstant(1788962400000)).toBe('2026-09-09T14:00:00.000Z');
    for (const junk of [null, undefined, 0, -1, NaN, '1788962400', {}]) {
      expect(quoteInstant(junk)).toBeNull();
    }
  });
});
...
describe('technicals — null-honest (hazard 1 / C-20)', () => {
  ...
  it('the label carries the quote time and the candle date — the two vintages the data has', () => {
    expect(section.label).toMatch(/^Technicals · last quote .+ · daily indicators as of Sep 8$/);
  });

  it('a fallback quote says so and drops the quote time it does not have', () => {
    const s = composeTechnicals(SHORT_WINDOW, { current: 149.5, fallback: true, candleDate: '2026-09-08' });
...
describe('the standing', () => {
  it('reads the ROW’s own fields for a held name, in the row’s own precedence', () => {
    const s = composeStanding(UNIVERSE_PLACE.BOOK, {
      tier: 'star', openPrice: 141.02, entryPrice: 999, swappedInAt: '2026-09-08T15:15:00.000Z',
    });
    expect(s.facts[0]).toBe('star');
    expect(s.facts).toContain('Entry $141.02');   // openPrice, what the board renders
    expect(s.facts.join(' ')).not.toMatch(/999/);
  });

  it('falls back to the battle’s deploy instant for held-since, as the row does', () => {
    const s = composeStanding(UNIVERSE_PLACE.BOOK, { tier: 'core' }, '2026-09-08T13:30:00.000Z');
  ...
  it('is the ruled absence line for a bench or watchlist name, with no facts', () => {
    expect(composeStanding(UNIVERSE_PLACE.BENCH, null)).toEqual({ place: 'bench', line: 'On the bench', facts: [] });
    expect(composeStanding(UNIVERSE_PLACE.WATCHLIST, null)).toEqual({ place: 'watchlist', line: 'On the watchlist', facts: [] });
